...
Write-Host "Checking orchestrator diagnostics..."
try {
  Invoke-RestMethod -Method Get -Uri "http://localhost:8088/api/v1/diagnostics" | ConvertTo-Json -Depth 8
} catch {
  Write-Warning "Diagnostics endpoint unavailable. Rebuild/restart orchestrator with V47 if you want detailed diagnostics."
}
...
Write-Host "Starting V47 Bandai neural overlay pipeline through Spring Boot API..."
Write-Host $body
