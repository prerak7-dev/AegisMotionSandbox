# Aegis AAA Animation Pipeline V38 — Motion Data Harvester

V38 extends V37 with a local webpage for collecting raw public motion data sources.
...
The V37 learned motion prior needs a real dataset. The harvester helps build the first source library from:
...
- LaFAN1
- Bandai Namco
- your own direct URLs/local files

The page includes source-specific fields and licensing warnings.

## Recommended sources for first 30–100 clips

1. CMU / CMU-FBX for permissive baseline motion.
2. ACCAD for kicks, runs, martial arts, and walks.
3. Your own authored Manny/Quinn animations or licensed packs that permit ML training.
4. 100STYLE for locomotion style if the license fits your use.
5. LaFAN1/Bandai Namco for non-commercial research experiments only, unless your legal review says otherwise.

Do not use Mixamo animations to train the model.

## Next step after fetching

Retarget every clip to UE5 Manny/Quinn, then export Aegis JSON. The learned motion prior trains on Aegis JSON, not raw BVH/FBX/ASF/AMC.
